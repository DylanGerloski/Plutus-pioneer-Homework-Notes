...
catchContract :: Contract () PaySchema Void ()
catchContract = Contract.handleError
...
payTrace a b = do
        h1 <- activateContractWallet (Wallet 1) catchContract
        callEndpoint @"pay" h1 $ PayParams
            { ppRecipient = pubKeyHash $ walletPubKey $ Wallet 2
            , ppLovelace  = a
            }
        void $ Emulator.waitNSlots 1
        h2 <- activateContractWallet (Wallet 1) catchContract
