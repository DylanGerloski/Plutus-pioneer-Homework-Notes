...
transition game s r = case (stateValue s, stateData s, r) of
    ...
    -- | given player one made a move, player 2 wants to makes a move of choice c
    --   must contain lovelaces staked by player 1
    --   must be signed by second player
    --   must play within deadline
    --   transitions to new datum state which contains choice of player 2 and adds stake of second player
    ...
    -- |  after both players move, player 2 realizes they won and shows that using reveal
    --    game dataum state must contain both each lovelace stake of player
    --    reveal must be signed by first player
    --    must claim before given deadline
    --    game has ended, goes to final state
    ...
    -- | after both moves, player 2 realizes it is draw and shows this using reveal
    --   choice of player one must equal that of player 2 and both players stakes must be present
    --   must add extra constain to give back player 2s stake
        | (lovelaces v == (2 * gStake game)) &&
    ...
    -- | second player does not move before deadline
    --   first player must sign
...
-- maperror' takes a SMContractError and turns it into a Text error
mapError' :: Contract w s SMContractError a -> Contract w s Text a
mapError' = mapError $ pack . show

-- given a time, waits untill this time has been reached and then waits another slot
waitUntilTimeHasPassed :: AsContractError e => POSIXTime -> Contract w s e ()
waitUntilTimeHasPassed t = void $ awaitTime t >> waitNSlots 1


     -- | Off-chain code begins with first player
     --   first we get our own public key hash
     --   then use getThreadToken to actually mint NFT to be used to identify correct UtxOs to grab ***make sure to use this function before any other transactions are made
     --   because this can change the UtxO hash
     --   use maperror` to convert contract error messages to text error messages
     --   next define our game
     --   then out gameClient using game client fucntioon
     --   v = stake, c = choice, bs = hash we use to reveal
     --   runInitialise client (GameDatum bs Nothing) v = first mints the NFT corresponding to thread token. 
     --   then creates utxo at adress and puts NFT in that UtxO
     --   tell $ Last $ Just tt is needed in order to tell the NFT so the second player knows where the thread token is
...
firstGame fp = do
    ...
    -- | wait until time has passed for 2nd player to move 
    --   then we getOnChainState and return something of Maybe onChainState
    --   onchainState consists of a tuple of TypedSciptTxOut and TypedScriptTxOutRef
    --   contains UtxO and refernce to it and also additoinally provides the datum
    --   use tyTxOutData o to directly access data in datum
    --   then must go over the 2 cases,
    --         if player 2 hasnt moved, player 1 claims
    --         else player 2 reveals
    --         use runStep to transition the state mission
    --         takes client and the redeemer 
